...
(*  Audit and repair of the deconfliction column of AllModelsS123.csv.
...
Comment@"Two rows of paper I, S123B1C1D1 and S123C1D1, recorded an EMPTY
deconfliction although models demonstrably lie below them; a coauthor supplied
conditions there by hand. Does a repaired algorithm recover them, what else
does it change, and does it reproduce the survey where the survey was right?";

Section@"Reading the survey output";

Code[
	CsvFile=FileNameJoin@{$ThisDirectory,"AllModelsS123.csv"};
...
Code[
	...
	SelfTestB=SameUnionQ[AsAlternativeList@RecordedDeconfliction@"S123C1E1",
			AsAlternativeList@RecordedDeconfliction@"S123C2E1"]];
...
Section@"The two rows corrected by hand";

Code[
	AlessandroB1C1D1={{K1+K2,M2},{K2,M2}};
	AlessandroC1D1={{K1+K2,M1},{7*K1+K2,M1},{6*K1+K2,M1},{4*K1+K2,M1}};
	Print["S123B1C1D1 recorded = ",InputForm@RecordedDeconfliction@"S123B1C1D1"];
	Print["           repaired = ",InputForm@SurveyFormOf@"S123B1C1D1"];
	Print["           matches the hand correction: ",
		SameUnionQ[SurveyFormOf@"S123B1C1D1",AlessandroB1C1D1]];
	Print["S123C1D1   recorded = ",InputForm@RecordedDeconfliction@"S123C1D1"];
	Print["           repaired = ",InputForm@SurveyFormOf@"S123C1D1"];
	Print["           matches the hand correction: ",
		SameUnionQ[SurveyFormOf@"S123C1D1",AlessandroC1D1]];
	,LineLabel->"HandCorrections"];
...
Code[
	...
	Print["MODELS CHANGED (",Length@Changed," of ",Length@ModelNames,"): ",Changed];
	Do[Print["   ",M,"  recorded = ",InputForm@AsAlternativeList@RecordedDeconfliction@M,
		"  repaired = ",InputForm@SurveyFormOf@M],{M,Changed}];
